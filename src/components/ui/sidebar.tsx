      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn(
              "relative w-full has-[[data-variant=inset]]:bg-sidebar",
              className
            )}
            ref={ref}
            {...props}
...
>(
  (
    ...
    const isOverDropdownRef = React.useRef(false);

    const handleMouseEnter = React.useCallback(() => {
      if (!isMobile && autoCollapse) {
        setOpen(true);
      }
    }, [isMobile, autoCollapse, setOpen]);

    const handleMouseLeave = React.useCallback((e: React.MouseEvent) => {
      if (!isMobile && autoCollapse) {
        // Kiểm tra xem chuột có đang hover vào dropdown không
        ...
        // Nếu không hover vào dropdown thì mới thu gọn sidebar
    ...
    React.useEffect(() => {
      ...
      const handleDropdownMouseLeave = () => {
        ...
        // Chỉ thu gọn nếu chuột không còn ở trên sidebar
        if (!document.querySelector('.sidebar:hover')) {
          setOpen(false);
        }
      };

      // Thêm event listeners cho dropdown menu
    ...
    return (
      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className="flex h-full w-full flex-col bg-sidebar rounded-2xl group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
          >
            {children}
          </div>
        </div>
...
>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        "relative flex min-h-svh flex-1 flex-col bg-white dark:bg-zinc-950 w-full",
