...
import { UserOutlined, ClockCircleOutlined, CheckCircleOutlined, CloseCircleOutlined, FileWordOutlined } from '@ant-design/icons';
...
const Approve: React.FC = () => {
  ...
  useEffect(() => {
    const fetchClaims = async () => {
      try {
        const response = await claimService.getClaims(claimStatus, pageNumber, pageSize, "ApproverMode");
  ...
  const columns: TableColumn[] = [
    ...
    },
    {
      ...
      render: (status) => {
        ...
        if (status === 'Rejected') color = '#ff7675';
        return (
    ...
    <div className="p-6">
      ...
      <Row gutter={24} className="mb-6">
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <div className="flex items-center gap-4 p-1">
              <div className="w-[65px] h-[65px] rounded-full flex items-center justify-center bg-[#fff3e6] text-[#f3b760]">
                <ClockCircleOutlined className="text-[35px]" />
              </div>
              <div className="flex flex-col mt-2">
                <span className="text-[#666] text-xs">Pending</span>
                <span className="text-[30px] font-bold text-[#333]">{statusCounts.pending}</span>
              </div>
            </div>
          </Card>
        </Col>
        ...
        <Col xs={24} sm={12} md={6}>
          ...
          >
            <div className="flex items-center gap-4 p-1">
              <div className="w-[65px] h-[65px] rounded-full flex items-center justify-center bg-[#ffd0d0] text-[#ff3636]">
                <CloseCircleOutlined className="text-[35px]" />
              </div>
              <div className="flex flex-col mt-2">
                <span className="text-[#666] text-xs">Rejected</span>
                <span className="text-[30px] font-bold text-[#333]">{statusCounts.rejected}</span>
              </div>
            </div>
          </Card>
        </Col>
